...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        z_k_temp = z_k.reshape(-1)
        z_k_N_temp = z_k_N.reshape(-1)
        # print("HERE ======================")
        sum_under_k = 0
        sum_under_k_N = 0
        for i in range(2*N):
            if i != k :
                sum_under_k += torch.exp(sim(z_k_temp, out[i].reshape(-1)) / tau)
            if i != k+N :
                sum_under_k_N += torch.exp(sim(z_k_N_temp, out[i].reshape(-1)) / tau)
        l1 = torch.exp(sim(z_k_temp, z_k_N_temp) / tau) / (torch.tensor(sum_under_k))
        l2 = torch.exp(sim(z_k_N_temp, z_k_temp) / tau) / (torch.tensor(sum_under_k_N))
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    sim_matrix = torch.matmul(out, out.T) / (torch.linalg.norm(out, dim=1).reshape(-1, 1) * torch.linalg.norm(out, dim=1).reshape(1, -1))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    positive_pairs = sim_positive_pairs(out_left, out_right)/tau
    # positive_pairs /= tau
    total_positive_pairs  = torch.concat([positive_pairs, positive_pairs], dim=0)
    total_positive_pairs = torch.exp(total_positive_pairs)
    ...
    numerator = None
    numerator = total_positive_pairs/ denom

    
    
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss = None
    loss = -torch.sum(torch.log(numerator)) / (2 * N)
