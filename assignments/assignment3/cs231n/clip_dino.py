...
def get_similarity_no_loop(text_features, image_features):
    ...
    ############################################################################
    t_feat_norm = text_features / text_features.norm(dim=-1, keepdim=True)
    i_feat_norm = image_features / image_features.norm(dim=-1, keepdim=True)
    similarity = t_feat_norm @ i_feat_norm.t()
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    preds = similarity.argmax(dim=0).cpu().tolist()
...
class CLIPImageRetriever:
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        sim = (text_features @ self.image_features.T).squeeze(0)
        top_indices = sim.topk(k).indices.tolist()
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return top_indices
...
class DINOSegmentation:
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        for i in range(num_iters):
            ...
            loss = self.criterion(scores, Y_train.long())
            loss.backward()
            self.optimizer.step()
        ############################################################################
        #                             END OF YOUR CODE                             #
