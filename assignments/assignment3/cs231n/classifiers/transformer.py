...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        features_embed = self.visual_projection(features)
        features_embed = features_embed.unsqueeze(1)
        
        tgt_mask = torch.tril(torch.ones(T, T)).view(1, T, T)
        
        decoder_output = self.transformer(captions_embed, features_embed, tgt_mask)
